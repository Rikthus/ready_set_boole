...
fn conjunctive_normal_form(formula: &str) -> String {
    let alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    let truth_table = generate_truth_table(formula);
    if truth_table.is_err() {
        println!("Invalid formula");
        return "".to_string();
    }
    // print_result(truth_table.unwrap());
    let mut cnf  = String::new();
    let mut nb_comb: usize = 0;

    for comb in &truth_table.unwrap() {
        if let Some(value) = comb.last() {
            if *value == false {
                nb_comb += 1;
                let nb_disjunction = comb.len() - 2;

                for (index, var) in comb.iter().enumerate() {
                    if index == comb.len() - 1 {
                        break;
                    }
                    if let Some(ch) = alphabet.chars().nth(index) {
                        if *var == true {
                            cnf.push(ch);
                            cnf.push('!');
                        } else {
...
fn main() {
    println!("Result {}", conjunctive_normal_form("AA!|"));
}
